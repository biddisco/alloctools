...
// an aid for alloctools development.
// ------------------------------------------------------------
...
//
// static alloctools::debug::enable_print<true> spq_deb("SUBJECT");
//
...
// ------------------------------------------------------------
/// \cond NODETAIL
namespace alloctools { namespace debug {
    ...
    struct mem_crc32
    {
        ...
        friend std::ostream& operator<<(std::ostream& os, mem_crc32 const& p)
        {
            ...
            os << "Memory:";
            os << " address " << alloctools::debug::ptr(p.addr_) << " length "
               << alloctools::debug::hex<6>(p.len_)
               << " CRC32:" << alloctools::debug::hex<8>(crc32(p.addr_, p.len_))
               << "\n";
            for (size_t i = 0;
                 i < (std::min)(size_t(std::ceil(p.len_ / 8.0)), size_t(128));
                 i++)
            {
                os << alloctools::debug::hex<16>(*uintBuf++) << " ";
            }
            os << " : " << p.txt_;
            return os;
        }
    ...
    namespace detail {
        ...
        // ------------------------------------------------------------------
        // helper class for printing thread ID, either std:: or alloctools::
        // ------------------------------------------------------------------
